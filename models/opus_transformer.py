...
class OPUSTransformer(BaseModule):
    ...
    def forward(self, query_points, query_feat,dn_points,dn_query,mlvl_feats, img_metas):
        cls_scores, refine_pts,dn_cls_scores, dn_refine_pts= self.decoder(
            query_points, query_feat, dn_points,dn_query,mlvl_feats, img_metas)

        dn_cls_scores = [torch.nan_to_num(score) for score in dn_cls_scores]
        dn_refine_pts = [torch.nan_to_num(pts) for pts in dn_refine_pts]
        cls_scores = [torch.nan_to_num(score) for score in cls_scores]
        refine_pts = [torch.nan_to_num(pts) for pts in refine_pts]

        return cls_scores, refine_pts, dn_cls_scores, dn_refine_pts
...
class OPUSTransformerDecoder(BaseModule):
    ...
    def forward(self, query_points, query_feat, dn_points,dn_query,mlvl_feats, img_metas):
        ...
        for i, decoder_layer in enumerate(self.decoder_layers):
            ...
            dn_points = dn_points.detach()
            #################################
            query_feat, cls_score, query_points, dn_query, dn_cls_score, dn_points = decoder_layer(
                query_points, query_feat, dn_points, dn_query, mlvl_feats, occ2img, img_metas)
            ###################################
            cls_scores.append(cls_score)
            refine_pts.append(query_points)
            dn_cls_scores.append(dn_cls_score)
            dn_refine_pts.append(dn_points)
        return cls_scores, refine_pts, dn_cls_scores, dn_refine_pts
...
class OPUSTransformerDecoderLayer(BaseModule):
    ...
    def forward(self, query_points, query_feat, dn_points, dn_query,mlvl_feats, occ2img, img_metas):
        """
        query_points: [B, Q, 3] [x, y, z]
        """
        
        query_pos = self.position_encoder(query_points.flatten(2, 3)) # 当query对应多个points时，这个Linear层怎么运行
        query_feat = query_feat + query_pos
        #################################################################
        dn_points = encode_points(dn_points,self.pc_range)
        dn_query_pos = self.position_encoder(dn_points.flatten(2,3))
        dn_query = dn_query + dn_query_pos
        
        dn_sampled_feat = self.sampling(
            dn_points, dn_query, mlvl_feats, occ2img, img_metas) # [B, Q, G, FP, C]
        dn_query = self.norm1(self.mixing(dn_sampled_feat, dn_query))
        dn_query = self.norm2(self.self_attn(dn_points, dn_query))
        dn_query = self.norm3(self.ffn(dn_query))
        
        dn_B, dn_Q = dn_points.shape[:2]
        dn_cls_score = self.cls_branch(dn_query)  # [B, Q, P * num_classes]
        dn_reg_offset = self.scale * self.reg_branch(dn_query)  # [B, Q, P * 3]
        dn_cls_score = dn_cls_score.reshape(dn_B, dn_Q, self.num_refines, self.num_classes)
        dn_refine_pt = self.refine_points(dn_points, dn_reg_offset)
        ##########################################################################################
        ...
        return query_feat, cls_score, refine_pt, dn_query, dn_cls_score, dn_refine_pt
...
class OPUSSampling(BaseModule):
    ...
    def inner_forward(self, query_points, query_feat, mlvl_feats, occ2img, img_metas):
        ...
        sampling_offset = self.sampling_offset(query_feat) # bs num_query 48
